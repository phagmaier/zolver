...
`SubgameManager` currently owns one cached truncated Flop solution plus its first chance-boundary seeds. `solveFlop` builds/solves a Flop tree truncated at Turn and then walks average strategies to collect `ChanceSeed` entries. Each seed carries a fixed-size action path of `PathStep` records (action + edge amount), so paths disambiguate seeds that share an `Action` sequence but differ in bet size — e.g., a half-pot `BET` + `CALL` and a full-pot `BET` + `CALL` produce distinct seeds. Callers select a seed via `findSeedByPath(&.{ .{ .action = .BET, .amount = 75 }, .{ .action = .CALL, .amount = 100 } })` / `solveTurnByPath(...)` rather than array order; `chanceSeeds()` exposes the full list for enumeration (e.g., for an interactive CLI). `solveTurn(seed_index, turn_card, ...)` masks private hands containing that public Turn card, applies the chance transition, and spins up a fresh Turn `Subgame` truncated at River. `solveRiverFromSeed` (or `solveRiverFromPath` for path-based lookup against a Turn's collected seeds) resolves a River subgame from a chance seed.

### Parallel CFR

`cfr.solve` runs in parallel by default when `cpu_count >= 2` and `iterations > 0`. Each strategy update spawns up to `MAX_PARALLEL_WORKERS = 8` workers via `std.Thread.spawn`. Each worker owns:

- its own `SolveContext` over its own `BoardContext` (so `reinitForBoard` and the chance-snapshot stack don't collide between workers),
- its own `WorkerDeltas`: per-decision-node `regret` and `strategy_sum` delta slices, allocated once at solve start from a caller-supplied allocator,
- its own `std.Random.DefaultPrng` seeded deterministically from the caller's `random` so chance-sample variance is independent across workers.

Workers all see the *same* `node.regrets` / `node.strategy_sum` snapshot during one update (regret matching reads the shared tree; the writes go to private deltas). After all workers join, `mergeDeltas` sums each worker's flat per-node delta list into the shared tree, applies the CFR+ non-negative clamp on regret, and zeroes each slot in the same pass so the next iter starts clean without a separate reset. The flat `WorkerDeltas.flat` list lets the merge avoid hashmap lookups in the hot per-iter aggregation loop.

`exploitability` runs the two `bestResponse` directions on two threads (each with its own `BoardContext` copy). `brWalk` additionally partitions the chance-runout enumeration across up to 8 workers at the first chance node it encounters (`SolveContext.allow_parallel` is cleared on spawned worker contexts so recursive chance descents stay serial — no exponential thread explosion).

The serial fallback (used by single-CPU systems and bypassed entirely when `iterations == 0`) is preserved unchanged: when `n_workers <= 1`, `solve` runs the original in-place walk with no deltas and no thread spawn.

## Current State

- **Performance:** `zig build test` runs in ~1m40s on an 8-core debug build (parallel CFR active). Total CPU time is ~10m, reflecting near-full parallel utilization on the solve-heavy tests. Serial baseline was ~66s; the small wall-clock overhead in debug is per-iter merge/reset bookkeeping and dwarfs the actual walk parallelism gain in unoptimized builds. Release builds (`-Doptimize=ReleaseFast`) should see meaningful wall-clock speedup; verification of that delta is future benchmark work.
- **Verification:** Convergence is verified on AA-vs-KK toy games (River and Turn) and a "Polarized vs Condensed" range test, with exploitability asserted `< 0.05` after 200 CFR+ iterations. The parallel solve produces lower-variance per-iter contributions (N samples averaged per strategy update) and still satisfies these bounds. Behavioral correctness is checked by asserting KK folds to a bet on the AA-vs-KK river via `averageStrategy`. Tree structure for pre-river all-in runouts and truncated chance leaves is verified by `node.zig` structural tests. Snapshot/restore round-trip is verified directly. Worker isolation is verified by the `two SolveContexts on separate BoardContexts isolate board state` test. Chance enumeration is checked for private-hand blocker denominators in both `allInEquityLeaf` and `brWalk`, and the walker leaf path is tested directly. `subgame.zig` verifies fresh solver construction from dense reaches, Flop chance-seed collection (with bet-sizing fan-out producing distinct seeds), Turn re-solver construction, Turn full-vs-truncated all-in response consistency, bounded compact truncated polarized/condensed exploitability, and truncated Flop tree-size reduction. `cfr.zig` verifies that the all-in equity leaf matches exact full-runout chance enumeration on AA-vs-KK.
...
1.  **Release-build perf characterization** — measure wall-clock speedup of parallel CFR against serial at `-Doptimize=ReleaseFast`. The debug-build overhead masks the parallelism benefit; a focused benchmark (outside the unit test suite) is needed to validate the expected ~N× scaling.
...
3.  **`allInEquityLeaf` runout caching** — for a truncated flop solve, the per-runout `BoardContext` (hand_strengths, sorted_indices, rank_map, first_rank, last_rank) is invariant across iterations. Pre-computing all 47×46 turn/river boards once and swapping cached `BoardContext` snapshots into the worker would eliminate the dominant inner cost (~430K hand-strength sorts per 200-iter solve). Memory cost is ~40 MB per flop, acceptable for personal-machine use.
4.  **Small cleanups when convenient:** make `HandTable.getIndex` non-linear if it ever lands in a hot loop, and consider heap-backed scratch buffers for `walk`/`brWalk` (~90 KB per non-chance frame is acceptable on the main thread but tight for arbitrary worker stack sizes).
5.  **CLI/UI (last):** Parse user-supplied ranges/boards, run a solve, print per-action probabilities via `averageStrategy`. Range input format: investigate whether a community standard exists (PioSOLVER text format, GTO+ JSON, etc.) before defining our own. Otherwise spec a minimal format. Also support file upload of a preflop range.
